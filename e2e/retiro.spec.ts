...
test.describe("Public retreat pre-registration", () => {
  ...
  test("unauthenticated GET /retiro shows the Spanish retreat form", async ({
    ...
    test.skip(
      ...
      "Chromium is enough for the public-form smoke",
    );
    ...
    await expect(
      page.getByText("Retiro juvenil", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Un Corazón Nuevo" }),
    ).toBeVisible();
    await expect(
      page.getByText(/Un espíritu nuevo · Ezequiel 36,26/),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("button", { name: "Preinscribirme al retiro" }),
    ).toBeVisible();

    await expect(
      page.getByText(/PREINSCRIPCIÓN AL RETIRO JUVENIL/),
    ).toBeVisible();
