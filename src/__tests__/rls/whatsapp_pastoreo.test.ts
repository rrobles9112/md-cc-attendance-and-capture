...
describe("whatsapp_pastoreo RLS contract", () => {
  it("notification_log SELECT policy exists and denies server/anon", async () => {
    // This will fail RED until migration 012c exists.
    // We assert the migration file is present and contains correct policy shape.
    const fs = await import("node:fs");
    const path = "supabase/migrations/012c_whatsapp_pastoreo_rls.sql";
    const exists = fs.existsSync(path);
    expect(exists, "012c RLS migration must exist (RED → GREEN)").toBe(true);
    if (!exists) return;
