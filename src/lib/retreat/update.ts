...
export type RetreatPersistResult =
  | { ok: true }
  | { ok: false; error: string }
...
export function mapRetreatUpdateError(error: { message?: string; code?: string }): string {
  const code = error.code ?? ""
  const message = error.message ?? ""
  if (code === "23505" || message.includes("already_preinscribed") || message.includes("duplicate")) {
...
export async function applyRetreatRegistrationUpdate(
  ...
  if (!data || data.length === 0) {
    return { ok: false, error: "No tiene permisos para editar esta preinscripción" }
  }
  return { ok: true }
}
