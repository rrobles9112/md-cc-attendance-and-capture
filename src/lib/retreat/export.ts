...
export function buildReportRows(
  ...
  return idOrder.map((id, idx) => {
    ...
    return {
      ...
      Estado: r.status,
      TieneCondicionMedica: r.has_medical_conditions ? "Sí" : "No",
      Condiciones: r.medical_conditions ?? "",
      Medicamentos: r.medical_medications ?? "",
      Dosis: r.medical_dosage ?? "",
      "Pagado COP": paid.toLocaleString("es-CO", {
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      }),
      "Saldo COP":
      ...
      PorcentajePagado:
        percent === null
          ? ""
          : `${percent.toLocaleString("es-CO", { maximumFractionDigits: 0 })}%`,
      "TotalRetiro COP": totalLabel,
      CantidadAbonos: pays.length,
      ÚltimoAbono: last
        ? new Date(last).toLocaleDateString("es-CO", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
          })
      ...
      FechaTransferencia: r.transferred_at
        ? new Date(r.transferred_at).toLocaleDateString("es-CO", {
            day: "2-digit",
            month: "2-digit",
            year: "numeric",
          })
...
export function exportRetreatToXLSX(
  rows: RetreatReportRow[],
  filename: string,
): void {
  if (rows.length === 0) {
  ...
  const ws = XLSX.utils.json_to_sheet(rows, {
    header: Object.keys(rows[0]) as (keyof RetreatReportRow)[],
  });
  XLSX.utils.sheet_add_aoa(ws, [[header]], { origin: "A1" });
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, "Retiro");
...
export function exportRetreatToCSV(
  rows: RetreatReportRow[],
  filename: string,
): void {
  const ws = XLSX.utils.json_to_sheet(rows);
